...
Przeniesienie pakietu do ROS'a w wersji Hydro wymaga³o miêdzy innymi zmiany rozwi±zania odno¶nie budowania paczek. Opis zawarto¶ci paczek w przypadku ró¿nych systemów budowania przedstawiono w \cite{ros_package}. W poprzedniej wersji pakietu  \emph{pmd\_camboard\_nano} u¿ywano \emph{rosbuild}\cite{http://wiki.ros.org/rosbuild}, natomiast w nowszej wersji wykorzystano ¶rodowisko \emph{catkin} oparte na wieloplatformowym systemie budowania pakietów \emph{cmake}. Odno¶nie systemu \emph{cmake}, jego zalet± jest to, ¿e jest niezale¿ny od platformy i kompilatora. W zasadzie nie kompiluje on programów samodzielnie, ale wykorzystuje do tego regu³y budowania pakietów specyficzne dla danej platformy. Przyk³adowo dla systemu linux, s± to pliki Makefile.  Warto zaznaczyæ, ¿e rozwi±zanie z rosbuild'em wykorzystywane by³o we wcze¶niejszych wersjach ROS'a, natomiast w nowszych odchodzi siê od jego u¿ycia na rzecz ¶rodowiska catkin. Proces migracji do nowszego systemu opisany zosta³ na stronie \cite{migration}. 
...
\begin{itemize}
	...
	\item \textsf{roslaunch pmd\_camboard\_nano pmd\_camboard\_nano.launch} - uruchomienie pakietu, z wykorzystaniem oprogramowania roslaunch, czyli plików uruchomieniowych,
	\item \textsf{rosrun image\_view image\_view image:=/camera/depth/image} \\- wy¶wietlenie obrazu g³êbi. Dwukrotne wyst±pienie image\_view zwi±zane jest z organizacj± pakietów w ROS'ie, co znaczy tyle, ¿e pierwszy parametr okre¶la metapaczkê, natomiast drugi paczkê, która zawiera siê w wymienionej wcze¶niej metapaczce. W przypadku, gdy pakiet nie nale¿y do ¿adnej metapaczki, podaje siê jego nazwê dwukrotnie.
